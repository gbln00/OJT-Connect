...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .qa-label { font-family: '{{ $cssFontName }} Condensed', sans-serif; font-size: 12px; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; color: var(--text2); }
         ...
         {{-- Import CSV --}}
           ...
           <a href="{{ route('coordinator.export.index') }}"
            ...
            <a href="{{ route('coordinator.notifications.index') }}"
                ...
                Notifications
                @php
                    $notifCount = \App\Models\TenantNotification::forRole('ojt_coordinator')->forUser(auth()->id())->unread()->count();
                @endphp
            ...
            <a href="{{ route('coordinator.support.index') }}"
                class="nav-item {{ request()->routeIs('coordinator.support.*') ? 'active' : '' }}">
                <svg width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
