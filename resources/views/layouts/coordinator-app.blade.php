    ...
    <nav class="sidebar-nav">
        ...
        <a href="{{ route('coordinator.applications.index') }}"
           class="nav-item {{ request()->routeIs('coordinator.applications.*') ? 'active' : '' }}">
            ...
            Applications
            @php
                $pendingAppCount = \App\Models\OjtApplication::where('status','pending')->count();
            @endphp
        ...
        <a href="{{ route('coordinator.reports.index') }}"
           class="nav-item {{ request()->routeIs('coordinator.reports.*') ? 'active' : '' }}">
            ...
            Weekly Reports
            @php
                $pendingReportsCount = \App\Models\WeeklyReport::where('status','pending')->count();
            @endphp
            @if($pendingReportsCount > 0)
                <span style="margin-left:auto;padding:1px 7px;background:var(--coral-dim);color:var(--coral);
                             border-radius:20px;font-size:10.5px;font-weight:700;">
                    {{ $pendingReportsCount }}
                </span>
            @endif
