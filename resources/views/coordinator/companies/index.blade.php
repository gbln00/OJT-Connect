...
<div class="stats-grid fade-up" style="grid-template-columns:repeat(3,1fr);">
    ...
    <div class="stat-card">
        <div class="stat-top"><div class="stat-icon coral">
...
<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:16px;flex-wrap:wrap;" class="fade-up fade-up-1">
    <form method="GET" action="{{ route('coordinator.companies.index') }}" style="display:flex;gap:8px;flex:1;flex-wrap:wrap;">
        ...
        @if(request()->hasAny(['search','status']))
            <a href="{{ route('coordinator.companies.index') }}" class="btn btn-ghost btn-sm">Clear</a>
        @endif
    </form>
    <a href="{{ route('coordinator.companies.create') }}" class="btn btn-primary btn-sm">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <div style="display:flex;align-items:center;gap:4px;">
                            <a href="{{ route('coordinator.companies.edit', $company) }}" class="btn btn-ghost btn-sm">Edit</a>
                            <form method="POST" action="{{ route('coordinator.companies.toggle', $company) }}">
                                @csrf @method('PATCH')
                                <button type="submit" class="btn btn-sm {{ $company->is_active ? 'btn-danger' : 'btn-approve' }}">
                                    {{ $company->is_active ? 'Deactivate' : 'Activate' }}
                                </button>
                            </form>
                            <form method="POST" action="{{ route('coordinator.companies.destroy', $company) }}"
                ...
                <tr>
                    <td colspan="6" style="text-align:center;padding:48px;color:var(--muted);">
                        No companies yet.
                        <a href="{{ route('coordinator.companies.create') }}" style="color:var(--crimson);text-decoration:none;font-weight:500;">Add one →</a>
                    </td>
                </tr>
                @endforelse
            </tbody>
