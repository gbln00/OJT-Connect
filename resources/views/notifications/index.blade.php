@php
    ...
    $typeColors = [
        ...
        'error'   => ['border'=>'rgba(248,113,113,0.3)',  'bg'=>'rgba(248,113,113,0.07)', 'color'=>'#f87171', 'dot'=>'#f87171'],
    ];
@endphp
...
<div style="background:rgba(45,212,191,0.07);border:1px solid rgba(45,212,191,0.2);
            color:var(--teal,#2dd4bf);padding:12px 16px;margin-bottom:16px;
...
<div class="card fade-up fade-up-1">
    <div class="card-header">
        ...
        <div style="display:flex;gap:8px;align-items:center;">
            ...
            <span style="display:inline-flex;align-items:center;gap:5px;padding:3px 10px;
                         ...
                         font-family:'DM Mono',monospace;font-size:10px;color:var(--gold-color,#c9a84c);">
            ...
            <form method="POST"
                  action="{{ route($rolePrefix . '.notifications.markAllRead') }}"
                  style="margin:0;">
                @csrf
            ...
            @endif
            <form method="POST"
                  action="{{ route($rolePrefix . '.notifications.clearRead') }}"
                  style="margin:0;">
                @csrf
    ...
    @if($notifications->isEmpty())
        <div style="text-align:center;padding:80px 20px;">
            <div style="width:56px;height:56px;border:1px solid var(--border2);
                        display:flex;align-items:center;justify-content:center;
                        margin:0 auto 20px;">
                <svg width="22" height="22" fill="none" stroke="var(--muted)"
            ...
            <div style="font-size:13px;color:var(--muted);font-family:'DM Mono',monospace;">
                // No notifications yet.
            </div>
        </div>
    @else
        @foreach($notifications as $notif)
        @php $tc = $typeColors[$notif->type] ?? $typeColors['info']; @endphp
        <div style="display:flex;align-items:flex-start;gap:14px;padding:14px 20px;
            ...
            {{-- Icon --}}
            <div style="width:32px;height:32px;flex-shrink:0;display:flex;
                        align-items:center;justify-content:center;
                        border:1px solid {{ $tc['border'] }};
                        background:{{ $tc['bg'] }};margin-top:1px;">
                @if($notif->type === 'success')
                <svg width="13" height="13" fill="none" stroke="{{ $tc['color'] }}"
            ...
            <div style="display:flex;gap:4px;flex-shrink:0;align-items:center;">
                ...
                <button onclick="markReadInline({{ $notif->id }}, this)"
                        ...
                        title="Mark read" style="padding:5px 8px;">
                    <svg width="11" height="11" fill="none" stroke="currentColor"
                         stroke-width="2.5" viewBox="0 0 24 24">
                        <polyline points="20,6 9,17 4,12"/>
                    </svg>
                ...
                <button onclick="deleteNotif({{ $notif->id }}, this)"
                        ...
                        title="Delete" style="padding:5px 8px;">
                    <svg width="11" height="11" fill="none" stroke="currentColor"
                         stroke-width="2" viewBox="0 0 24 24">
                        <polyline points="3,6 5,6 21,6"/>
                        <path d="M19,6l-1,14a2,2,0,0,1-2,2H8a2,2,0,0,1-2-2L5,6"/>
...
const ROLE       = '{{ $rolePrefix }}';
const BASE_URL   = `/${ROLE}/notifications`;
...
function markReadInline(id, btn) {
    fetch(`${BASE_URL}/${id}/read`, {
        ...
        const dot = row.querySelector('span[style*="var(--crimson)"]');
        if (dot) dot.remove();
    });
}
